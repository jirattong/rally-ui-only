  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Analytics', style: TextStyle(fontWeight: FontWeight.w900, color: dark)),
      ),
      body: ListView(
    ...
    return Ink(
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Last Recent Summary', style: TextStyle(fontWeight: FontWeight.w800, color: Colors.black87)),
...
class _MetricBig extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Expanded(
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(number, style: const TextStyle(fontSize: 28, fontWeight: FontWeight.w900)),
  ...
  Widget build(BuildContext context) {
    ...
    return Ink(
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Motion', style: TextStyle(fontWeight: FontWeight.w800, color: Colors.black87)),
...
class _SimpleLineChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final axis = Paint()..color = const Color(0xFF754A2E)..strokeWidth = 1;
    // axes
    final margin = 24.0;
    final chart = Rect.fromLTWH(margin, margin, size.width - margin*2, size.height - margin*2);
    // horizontal lines
    for (int i = 0; i <= 4; i++) {
      final y = chart.top + chart.height * i / 4;
      canvas.drawLine(Offset(chart.left, y), Offset(chart.right, y), axis..color = const Color(0xFFB98E72));
    }
    ...
    final line = Paint()..color = const Color(0xFFFF7E2F)..strokeWidth = 2.5..style = PaintingStyle.stroke;
    ...
    for (int i = 0; i < points.length; i++) {
      ...
      // bar under point i
      final barRect = Rect.fromLTWH(p.dx - chart.width/(points.length*2.2), p.dy, chart.width/(points.length*1.1), chart.bottom - p.dy);
...
class _SessionHistoryPreview extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 4),
                    Text('$duration    $motion', style: const TextStyle(color: Colors.black87)),
                  ],
                ),
              ),
              TextButton(
                onPressed: () {
                  ...
                  Navigator.pushNamed(context, '/Analytics/detail', arguments: {
                    'date': date, 'duration': duration, 'motion': motion,
                  });
                },
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      const Text('Session History', style: TextStyle(fontSize: 24, fontWeight: FontWeight.w900)),
      const SizedBox(height: 12),
      ...
      Center(
        child: TextButton(
          ...
          child: const Text('View More', style: TextStyle(decoration: TextDecoration.underline)),
        ),
      ),
    ]);
  }
...
class AnalyticsHistoryPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: bg, elevation: 0,
        title: const Text('Session History', style: TextStyle(fontWeight: FontWeight.w900, color: Color(0xFF3A5150))),
      ...
      body: ListView.separated(
        ...
        itemBuilder: (context, i) {
          final s = sessions[i];
          return Ink(
            decoration: BoxDecoration(color: const Color(0xFFFBE0CC), borderRadius: BorderRadius.circular(16)),
            child: Padding(
              padding: const EdgeInsets.fromLTRB(14, 12, 8, 12),
              child: Row(children: [
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('${s['date']}', style: const TextStyle(fontWeight: FontWeight.w900)),
                  const SizedBox(height: 4),
                  Text('${s['duration']}    ${s['motion']}', style: const TextStyle(color: Colors.black87)),
                ])),
                TextButton(
                  onPressed: () => Navigator.pushNamed(context, '/Analytics/detail', arguments: s),
                  child: const Text('View Detail'),
                ),
              ]),
            ),
...
class AnalyticsDetailPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: bg, elevation: 0,
        title: const Text('Session Detail', style: TextStyle(fontWeight: FontWeight.w900, color: Color(0xFF3A5150))),
        leading: IconButton(icon: const Icon(Icons.arrow_back, color: Colors.deepOrange), onPressed: () => Navigator.pop(context)),
      ),
      body: ListView(
        padding: const EdgeInsets.fromLTRB(16, 8, 16, 16),
        children: [
          ...
          Ink(
            decoration: BoxDecoration(color: const Color(0xFFFBE0CC), borderRadius: BorderRadius.circular(16)),
            child: SizedBox(height: 160, child: CustomPaint(painter: _SimpleLineChartPainter(points: [3,5,4,6,5,7], maxY: 10))),
          ),
          const SizedBox(height: 12),
          ...
          Ink(
            ...
            child: Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: const [
                _MetricRow(icon: Icons.flash_on, label: 'Peak intensity', value: 'High'),
                SizedBox(height: 8),
                _MetricRow(icon: Icons.av_timer, label: 'Avg interval', value: '3.2 sec'),
                SizedBox(height: 8),
                _MetricRow(icon: Icons.speed, label: 'Consistency', value: '87%'),
                SizedBox(height: 8),
                _MetricRow(icon: Icons.sports_tennis, label: 'Top motion', value: 'Smash Ready'),
              ]),
            ),
          ),
        ],
